...
const TOPIC_COMPUTERS = {
    ...
    relativity(p) {
        const v = p.relativity?.velocity, c = 299792458;
        eq(p, 'γ = 1/√(1−v²/c²)'); eq(p, 'Δt′ = γΔt'); eq(p, 'L′ = L/γ'); eq(p, 'E = mc²');
        if (n(v) && v < c) {
            const beta = v / c, gamma = 1 / Math.sqrt(1 - beta * beta);
            calc(p, 'lorentz_factor', gamma); calc(p, 'beta', beta);
            if (n(p.time)) { calc(p, 'time_dilation', gamma * p.time); if (!n(p.relativity?.time_dilation)) p.relativity.time_dilation = r(gamma * p.time); }
