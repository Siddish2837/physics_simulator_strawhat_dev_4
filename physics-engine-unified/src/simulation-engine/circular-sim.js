...
import { drawDot, drawArrow, drawLine, drawHUD, getCamera, formatLength } from './simulation-core.js';
...
export function simulateCircularMotion(params, sim) {
    ...
    sim.addObject({
        ...
        render(ctx_r, canvas) {
            ...
            drawArrow(ctx_r, this.px, this.py, this.px + vx * arrowScale, this.py + vy * arrowScale, '#4ecdc4', `v=${formatLength(speed)}/s`);

            // Acceleration (centripetal)
            const dx = cx - this.px, dy = cy - this.py;
            const dist = Math.sqrt(dx * dx + dy * dy) || 1;
            const acLen = 40;
            drawArrow(ctx_r, this.px, this.py, this.px + (dx / dist) * acLen, this.py + (dy / dist) * acLen, '#ff6b6b', `ac=${formatLength(ac)}/s²`);

            // Radius label
            ctx_r.save();
            ctx_r.font = '11px "JetBrains Mono", monospace';
            ctx_r.fillStyle = 'rgba(255,255,255,0.3)';
            ctx_r.fillText(`r=${formatLength(r)}`, cx + r * scale / 2, cy - 6);
            ctx_r.restore();
            ...
            drawHUD(ctx_r, [
                ...
                `ω = ${omega.toFixed(2)} rad/s   r = ${formatLength(r)}`,
                `v = ${formatLength(speed)}/s`,
                `T = ${T.toFixed(3)} s   f = ${fc.toFixed(3)} Hz`,
                `ac = ${formatLength(ac)}/s²`,
            ]);
        }
    });
}
