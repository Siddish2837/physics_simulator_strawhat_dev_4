...
function lensSimulation(params, sim) {
    ...
    // Use the engine's global unit scale for stability, fallback to auto-scaling if not set
    // This respects the "without changing unit scale" request
    const s = sim.camera.unitScale * (sim.camera.zoom || 1);
    ...
    sim.addObject({
        ...
        render(ctx, canvas) {
            ...
            const imgLabel = `Image (M=${mag.toFixed(2)}, dᵢ=${formatLength(dI)})`;
            ctx.fillText(imgLabel, imgX - 20, cy - imgH + (imgH > 0 ? -12 : 20));
            ctx.restore();
