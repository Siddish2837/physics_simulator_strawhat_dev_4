...
export function setUnit(unit) {
    switch (unit) {
        case 'mm': camera.unitScale = 1000; camera.unitLabel = 'mm'; break;
        case 'cm': camera.unitScale = 100; camera.unitLabel = 'cm'; break;
...
/**
 * Format a length value according to the current unit scale.
 * @param {number} val - Value in meters
 * @param {number} decimals - Number of decimal places (default 2)
 * @returns {string} - Formatted string (e.g., "50.00 cm")
 */
export function formatLength(val, decimals = 2) {
    return `${(val * camera.unitScale).toFixed(decimals)} ${camera.unitLabel}`;
}
...
export function load(simFunction, params) {
    ...
    if (activeSimulation) {
        activeSimulation(params, {
            canvas, ctx, objects: simObjects,
            addObject, getTime: () => simTime, camera,
            worldToScreen, registerObject: registerTooltipObject,
            stop, start, reset
        });
    }
    start();
...
export function reset() {
    ...
    if (activeSimulation && currentParams) {
        activeSimulation(currentParams, {
            canvas, ctx, objects: simObjects,
            addObject, getTime: () => simTime, camera,
            worldToScreen, registerObject: registerTooltipObject,
            stop, start, reset
...
/**
 * Register an object for tooltip detection
 */
export function registerTooltipObject(obj) {
    if (!window.__simTooltips) window.__simTooltips = [];
    window.__simTooltips.push(obj);
}

/**
 * Clear tooltip objects (called on load/reset)
 */
export function clearTooltipObjects() {
    window.__simTooltips = [];
}
...
export function drawLabel(ctx, x, y, text, color = '#ffffff') {
    ...
    ctx.beginPath();
    drawRoundRect(ctx, x + 6, y - 16, w, 18, 4);
    ctx.fill();
    ctx.fillStyle = color;
...
export function drawHUD(ctx, lines, x = 24, y = 24) {
    ...
    ctx.beginPath();
    drawRoundRect(ctx, x, y, maxW, h, 8);
    ctx.fill();
