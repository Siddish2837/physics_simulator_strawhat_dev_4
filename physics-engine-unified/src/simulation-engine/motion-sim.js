...
import { drawDot, drawArrow, drawTrail, drawGround, drawHUD, toCanvas, getCamera, formatLength } from './simulation-core.js';
...
export function simulateLinearMotion(params, sim) {
    ...
    sim.addObject({
        ...
        render(ctx, canvas) {
            ...
            const vScale = 2.5;
            drawArrow(ctx, this.cx, this.cy - 25, this.cx + this.v * vScale, this.cy - 25, '#4ecdc4', `v = ${formatLength(this.v)}/s`);
            if (Math.abs(a) > 0.01) {
                drawArrow(ctx, this.cx, this.cy + 25, this.cx + a * vScale * 4, this.cy + 25, '#ff6b6b', `a = ${formatLength(a)}/s²`);
            }
            ...
            ctx.fillText(`Δx = ${formatLength(this.x)}`, (startX + this.cx) / 2 - 25, 466);
            ctx.restore();

            drawHUD(ctx, [
                `${params.object || 'Object'} — Linear Motion`,
                `t = ${this.t.toFixed(2)} s`,
                `x = ${formatLength(this.x)}   v = ${formatLength(this.v)}/s`,
                `x = x₀ + v₀t + ½at²`,
                a !== 0 ? `a = ${formatLength(a)}/s²` : `Uniform velocity`,
            ]);
        }
    });
}
