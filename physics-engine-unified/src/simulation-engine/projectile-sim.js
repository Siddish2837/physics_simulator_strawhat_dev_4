...
import { drawDot, drawArrow, drawTrail, drawLine, drawGround, drawHUD, toCanvas, getCamera, formatLength } from './simulation-core.js';
...
export function simulateProjectile(params, sim) {
    ...
    sim.addObject({
        ...
        render(ctx, canvas) {
            ...
            ctx.fillText(`↕ H = ${formatLength(maxH)}`, mhp.x + 6, mhp.y - 4);
            ctx.restore();
            ...
            ctx.fillText(`↔ R = ${formatLength(range)}`, (lp.x + rp.x) / 2 - 30, 520);
            ctx.restore();
            ...
            // Velocity components
            const vs = 1.8;
            drawArrow(ctx, this.cx, this.cy, this.cx + this.currentVx * vs, this.cy, '#4ecdc4', `vx ${formatLength(this.currentVx)}/s`);
            drawArrow(ctx, this.cx, this.cy, this.cx, this.cy - this.currentVy * vs, '#e056fd', `vy ${formatLength(this.currentVy)}/s`);

            // Resultant velocity
            const speed = Math.sqrt(this.currentVx ** 2 + this.currentVy ** 2);
            drawArrow(ctx, this.cx, this.cy, this.cx + this.currentVx * vs, this.cy - this.currentVy * vs, '#ffffff', `v ${formatLength(speed)}/s`);
            ...
            // HUD
            drawHUD(ctx, [
                `${params.object || 'Projectile'} — v₀=${formatLength(v0)}/s  θ=${angle}°`,
                `t = ${(this.t || 0).toFixed(2)} s`,
                `x = ${formatLength(this.x || 0)}   y = ${formatLength(this.y || 0)}`,
                `Range = ${formatLength(range)}   H_max = ${formatLength(maxH)}`,
