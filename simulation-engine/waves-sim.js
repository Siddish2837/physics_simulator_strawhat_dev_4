...
import { drawDot, drawLine, drawHUD, getCamera } from './simulation-core.js';
...
export function simulateWave(params, sim) {
    ...
    sim.addObject({
        ...
        render(ctx, canvas) {
            ...
            ctx.fillText(`A=${A} ${cam.unitLabel}`, mx + 8, (baseY + peakY) / 2);
            ctx.restore();
            ...
            if (xEnd - xStart > 20) { // Only draw if visible enough
                ...
                ctx.font = '11px "JetBrains Mono", monospace';
                ctx.fillText(`λ=${lambda} ${cam.unitLabel}`, (xStart + xEnd) / 2, yMark + 15);
                ctx.restore();
            }
            ...
            drawHUD(ctx, [
                ...
                `y = A·sin(2πft − kx)`,
                `A = ${A} ${cam.unitLabel}   f = ${f} Hz   λ = ${lambda} ${cam.unitLabel}`,
                `v = ${speed.toFixed(1)} ${cam.unitLabel}/s   T = ${(1 / f).toFixed(3)} s`,
            ]);
        }
    });
}
