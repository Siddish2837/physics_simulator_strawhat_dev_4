...
import { drawDot, drawLine, drawHUD, drawGround, getCamera } from './simulation-core.js';
    ...
    sim.addObject({
        ...
        render(ctx, canvas) {
            ...
            for (let hi = 0; hi <= hStart; hi += step) {
                ...
                ctx.fillText(`${hi.toFixed(1)}${cam.unitLabel}`, rulerX - 35, y + 3);
            }
            ctx.restore();
            ...
            drawHUD(ctx, [
                `Energy Conservation`,
                `Mass = ${m} kg`,
                `Height = ${this.h.toFixed(2)} ${cam.unitLabel}`,
                `Velocity = ${Math.abs(this.v).toFixed(2)} ${cam.unitLabel}/s`,
